...
class ControlPanel:
    def __init__(self, world, screen_width=1200, screen_height=900):
        ...
        # Sistema de herramientas para editar celdas
        self.tools = {
            'mutation': {'name': 'Mutation', 'icon': 'M', 'color': (255, 100, 100)},
            'energy': {'name': 'Energy', 'icon': 'E', 'color': (100, 255, 100)},
            'food': {'name': 'Food', 'icon': 'F', 'color': (100, 100, 255)},
        }
        self.active_tool = 'mutation'  # herramienta activa
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # Cambiar herramienta activa (1/2/3)
            if event.key == pygame.K_1:
                self.active_tool = 'mutation'
            elif event.key == pygame.K_2:
                self.active_tool = 'energy'
    ...
    def draw(self, screen, font, fps):
        ...
        # Menú de herramientas (abajo izquierda)
        y_offset = self.screen_height - 110
        for i, (tool_name, tool_info) in enumerate(self.tools.items()):
            is_active = self.active_tool == tool_name
            color = tool_info['color'] if is_active else (100, 100, 100)
            text = f"[{i+1}] {tool_info['name']}"
        ...
        if self.world.selected_cell:
            ...
            props = self.world.cell_grid[(cx, cy)]
            cell_text = f"Cell ({cx},{cy}): Mut={props['mutation_factor']:.1f}x Eng={props['energy_cost']:.1f}x Food={props['food_generation']:.1f}x"
